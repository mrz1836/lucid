...
func TestSpine_AllVerbsRegistered(t *testing.T) {
	...
	want := []string{"init", "log", "attach", "closeout", "mode", "status", "obs", "day", "validate", "export", "version", "upgrade", "scheduler", "anchor", "metrics", "storm", "profile", "person", "bootstrap", "serve", "reflect", "ask"}
	got := map[string]bool{}
