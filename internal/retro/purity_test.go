...
// TestRetroIsPure_NoIONoLLM enforces the record family's binding constraints by
// construction (retro.md §1: agent-free, no LLM in any path, and no filesystem
...
// whose logical-day rollover primitives this package reuses so the retro day key
// stays aligned with every other dated capture.
func TestRetroIsPure_NoIONoLLM(t *testing.T) {
	...
	allowed := map[string]bool{
		...
		// "no io, no LLM" intact while aligning the retro day key with the join.
	...
	for _, e := range entries {
		...
		for _, imp := range f.Imports {
			require.Truef(t, allowed[imp.Path.Value],
				"retro file %s imports %s — the record family must be pure (no io, no LLM/provider/agent)",
				name, imp.Path.Value)
		}
		scanned++
	}
	require.Positive(t, scanned, "expected to scan at least one retro source file")
}
