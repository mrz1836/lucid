...
| `/person merge\|alias\|rename\|set\|off-limits` | Deterministic, no-LLM curation of the people records ([`data-model.md`](data-model.md) §"Merge & redirect"): `merge` redirects a duplicate key onto a canonical one (append-only tombstone, never a delete), `alias` records another written form, `rename` changes the display name (key stable), `set` records the user-authored `dob`/`relationship`/`notes`, `off-limits` toggles P-3 redaction. Every rejection is decided before any write ([`error-states.md`](error-states.md) §P-4..P-9). | `people/` (+ `off_limits.json` for `off-limits`) |
| `/person reconcile` | Read-only, no-LLM detector: lists likely-duplicate people — every suggestion requires a corroborating signal (shared normalized aka/name, a nickname-map link, or a diminutive link; bare edit-distance proximity and entry co-occurrence are excluded) — with a suggested merge direction; changes nothing. Byte-stable across runs. See [`data-model.md`](data-model.md) §"Alias-aware person resolution". | None |
