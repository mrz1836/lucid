...
| `lucid recall [--era\|--thread\|--injury\|--pet <name\|key>]` | **Read-only:** browse the archive by era/thread/injury/pet (bare = an index); each dimension flag accepts a **name or key** (a case-insensitive, alias-aware human name or the opaque key), each surfaced item cited with its source context ([`life-archive.md`](life-archive.md) §7). | None |
