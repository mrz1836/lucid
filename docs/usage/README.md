...
| [`weekly-reflection.md`](weekly-reflection.md) | The read-only weekly deep-dive (`lucid reflect week`) — the catch-up window that covers every day since your last reflection (with its range overrides, cap, and `reflect week close` cursor), the projection-only bundle, the frameworks/lens layer and `provenance.framework` labeling, and the resonance-gated `reflect week apply` path that turns a confirmed pattern into a tracked insight. |
| [`witness-report.md`](witness-report.md) | The optional weekly witness report — the friend-facing accountability post (`lucid witness report`): the witness channel's purpose, the Monday cadence, the structural privacy boundary, the deterministic numbers + model prose split, the `witness_report` config block, the preview→auto gate, and the rich-embed layout that retires the monthly heartbeat. |
| [`life-archive.md`](life-archive.md) | Excavating the past — the injury/body-history and fun/story tracks: the write verbs (`lucid injury`/`era`/`thread`/`memory`, with backdating and optional `lucid attach` media) and the read-only `lucid excavate` selection + `lucid recall` browse, plus the harness-driven excavation ritual. |
